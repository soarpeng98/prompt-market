...
async function renderHome(category) {
  var app = document.getElementById("app");
  app.innerHTML = '<div class="hero"><h1>🪄 发现优质 AI 提示词</h1><p>覆盖文案、设计、编程、教育等 7 大分类</p><p style="font-size:13px;margin-top:4px">创作者发布赚钱 · 用户一键复制使用</p></div><div style="text-align:center;margin-bottom:20px"><span class="cat-tab active" id="sort-new" onclick="setSort(\'newest\')">🆕 最新</span><span class="cat-tab" id="sort-popular" onclick="setSort(\'popular\')">🔥 最热</span><span class="cat-tab" id="sort-free" onclick="setSort(\'free\')">🆓 免费</span></div><div class="search-bar"><span class="search-icon">🔍</span><input type="text" id="search-input" placeholder="搜索提示词..."></div><div class="categories" id="cat-tabs"><span class="cat-tab'+(category?'':' active')+'" data-cat="" onclick="filterByCategory(\'\')">🔥 全部</span>'+CATEGORIES.map(function(c){return '<span class="cat-tab'+(category===c.id?' active':'')+'" data-cat="'+c.id+'" onclick="filterByCategory(\''+c.id+'\')">'+c.icon+' '+c.name+'</span>';}).join("")+'</div><div id="prompt-grid" class="grid"><div class="skeleton-card"><div class="skeleton sk-cover"></div><div class="skeleton sk-line"></div><div class="skeleton sk-line short"></div><div class="skeleton sk-line medium"></div><div><div class="skeleton sk-tag"></div><div class="skeleton sk-tag"></div></div></div><div class="skeleton-card"><div class="skeleton sk-cover"></div><div class="skeleton sk-line"></div><div class="skeleton sk-line short"></div><div class="skeleton sk-line medium"></div><div><div class="skeleton sk-tag"></div><div class="skeleton sk-tag"></div></div></div><div class="skeleton-card"><div class="skeleton sk-cover"></div><div class="skeleton sk-line"></div><div class="skeleton sk-line short"></div><div class="skeleton sk-line medium"></div><div><div class="skeleton sk-tag"></div><div class="skeleton sk-tag"></div></div></div><div class="skeleton-card"><div class="skeleton sk-cover"></div><div class="skeleton sk-line"></div><div class="skeleton sk-line short"></div><div class="skeleton sk-line medium"></div><div><div class="skeleton sk-tag"></div><div class="skeleton sk-tag"></div></div></div></div></div>';
...
async function loadPrompts(category, search, append) { currentCategory = category||"";
  ...
  try{
    ...
    currentPage++;
    var searchInfo=search?'<div style="text-align:center;margin-bottom:12px;color:#94a3b8;font-size:13px">🔍 <b>'+escapeHtml(search)+'</b> 共 <b>'+allPrompts.length+'</b> 条</div>':'';
    if(allPrompts.length===0){grid.innerHTML='<div class="empty" style="grid-column:1/-1"><div class="icon">📭</div><p>暂无提示词</p><p style="font-size:13px">成为第一个发布的人！</p></div>';return;}
    grid.innerHTML=searchInfo+allPrompts.map(function(p){return '<a href="#/prompt/'+p.id+'" class="card">'+(p.cover_url?'<div class="card-cover" style="background-image:url('+escapeHtml(p.cover_url)+')"></div>':'')+'<div class="card-cat">'+getCatIcon(p.category)+' '+getCatName(p.category)+'</div><h3>'+escapeHtml(p.title)+'</h3><div class="card-desc">'+escapeHtml(p.description||"暂无描述")+'</div><div class="card-meta"><span class="card-price'+(p.price===0?' free':'')+'">'+(p.price===0?'免费':'¥'+(p.price/100).toFixed(2))+'</span><span class="card-stats">📥 '+(p.downloads||0)+' ⭐ '+(p.rating||0)+'</span></div><div class="platform-tags">'+(p.platforms||[]).map(function(pl){return '<span class="ptag">'+pl+'</span>';}).join("")+'</div><div class="tag-row">'+(p.tags||[]).slice(0,3).map(function(t){return '<span class="tag-badge" onclick="event.preventDefault();filterByTag(\''+t+'\')">#'+escapeHtml(t)+'</span>';}).join("")+'</div></a>';}).join("");
...
function copyPromptById(promptId){getPrompt(promptId).then(function(p){if(p)navigator.clipboard.writeText(p.content).then(function(){showToast("📋 已复制到剪贴板");});});}
...
async function toggleFeaturedAndGo(promptId) {
  var result = await toggleFeatured(promptId);
  if (result !== false) {
    showToast(result ? "✅ 已设为精选" : "已取消精选");
